...
We will first ask you to extract keypoints from both images using SIFT. For that, we refer you to the skeleton code in the \texttt{src} folder named \texttt{track\_features.cpp}. Follow the instructions written in the comments; specifically, you will need to complete:

\begin{itemize}
  \item The stub {\small \ttfamily SiftFeatureTracker::detectKeypoints()} and {\small \ttfamily SiftFeatureTracker::describeKeypoints()} in {\small \ttfamily lab5/feature\_tracking/src/sift\_feature\_tracker.cpp}
  \item The first part of {\small \ttfamily FeatureTracker::trackFeatures()} in {\small \ttfamily lab5/feature\_tracking/src/feature\_tracker.cpp}
\end{itemize}
...
\begin{enumerate}
  ...
  \item You might notice that naively matching features results in a significant amount of false positives (outliers). There are different techniques to minimize this issue. The one proposed by the authors of SIFT was to calculate the best two matches for a given descriptor and calculate the ratio between their distances: {\small \ttfamily Match1.distance < 0.8 * Match2.distance}. This ensures that we do not consider descriptors that have been ambiguously matched to multiple descriptors in the target image. Here we used the threshold value that the SIFT authors proposed (0.8).
...
\begin{itemize}
  \item The stub {\small \ttfamily SiftFeatureTracker::matchDescriptors()} in {\small \ttfamily feature\_tracking/src/sift\_feature\_tracker.cpp}
  \item The second part of {\small \ttfamily FeatureTracker::trackFeatures()} in {\small \ttfamily feature\_tracking/src/feature\_tracker.cpp}
  ...
  {\small \color{gray}
    ...
    Likewise, \texttt{good\_matches} is a pointer so to add to it you will need \texttt{good\_matches->push\_back(...)}.
  }
\end{itemize}
...
\begin{enumerate}
  ...
  {\small \color{gray}
    Hint: Note that \texttt{FeatureTracker::inlierMaskComputation} computes an inlier mask of type \texttt{std::vector<uchar>}, but for \texttt{cv::drawMatches} you will need a \texttt{std::vector<char>}. You can go from one to the other by using:

    {\ttfamily std::vector<char> char\_inlier\_mask{inlier\_mask.begin(), inlier\_mask.end()};}

    Hint: You will need to call the \texttt{cv::drawMatches} function twice, first to plot the everything in red (using \texttt{cv::Scalar(0,0,255}) as the color), and then again to plot inliers in green (using the inlier mask and \texttt{cv::Scalar(0,255,0)}). The second time, you will need to use the \texttt{DrawMatchesFlags::DRAW\_OVER\_OUTIMG} flag to draw on top of the first output. To combine flags for the \texttt{cv::drawMatches} function, use the bitwise-or operator:
...
{\small \color{gray} Hint: You may need to change your plotting code in \texttt{FeatureTracker::trackFeatures} to call \texttt{cv::waitKey(10)} instead of \texttt{cv::waitKey(0)} after imshow in order to get the video to play continuously instead of frame-by-frame on every keypress.}
...
\begin{enumerate}
  ...
  {\small \color{gray} Hint: You will need to convert the image to grayscale with \texttt{cv::cvtColor} and will want to look into the documentation for \texttt{cv::goodFeaturesToTrack} and \texttt{cv::calcOpticalFlowPyrLK}. The rest of the trackFeatures() function should be mostly familiar feature matching and inlier mask computation similar to the previous sections. Also note that the \texttt{status} vector from calcOpticalFlowPyrLK indicates the matches.}

  {\small \color{gray} Hint: For the show() method, you will just need to create a copy of the input frame and then make a loop that calls \texttt{cv::line} and \texttt{cv::circle} with correct arguments before calling \texttt{imshow}.}
\end{enumerate}
